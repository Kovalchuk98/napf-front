import { r as resizeCurrentAccordionHeight } from "../common_scripts/common_scripts.js";
class UploadFilesUtility {
  ...
  findFile(file) {
    return this.uploadFiles.find(function(existingFile) {
      return existingFile.name === file.name && existingFile.lastModified === file.lastModified && existingFile.size === file.size && existingFile.type === file.type;
    });
  }
...
document.querySelectorAll(".upload_input_file");
let uploadInputNames = document.querySelectorAll(".upload_input_name");
document.querySelectorAll(".materials_list");
...
sidebarMenuItems.forEach((sidebarMenuItem) => {
  sidebarMenuItem.addEventListener("click", () => {
    sidebarMenuItems.forEach((sidebarMenuItem2) => {
      sidebarMenuItem2.classList.remove("active_menu_item");
    });
...
publicateButtons.forEach((publicateButton) => {
  publicateButton.closest(".accordion");
  publicateButton.closest(".accordion_header");
...
function formatFileSize(bytes) {
  if (bytes == 0)
    return "0 Bytes";
  let k = 1e3, dm = 0, sizes = ["б", "кб", "мб", "гб", "тб"], i = Math.floor(Math.log(bytes) / Math.log(k));
...
uploadButtons.forEach((uploadButton) => {
  ...
  uploadButton.addEventListener("click", function(e) {
    ...
    if (!uploadWrapper.className.includes("mail_upload_edit")) {
      ...
      if (file) {
        let file_ext = file == null ? void 0 : file.name.split(".").pop();
        ...
        documentEditButton.addEventListener("click", (e2) => {
          editDocument(documentEditButton);
        });
        documentRemoveButton.addEventListener("click", (e2) => {
