...
import '../../src/styles/personal_area/mailings/index.scss';
...
// flatpicker.addEventListener('change', (event) => {
...
function calcCounterTags() {
  ...
  // if (!isClamped) {
  //   moreTagsButtons[0].style.display = 'none';
  // }
...
function isTextOverflowing(element) {
  ...
  // Check if the clone is overflowing
  ...
  // Store the full text and start with an empty string in the clone
  const fullText = element.textContent || element.innerText;
  clone.textContent = '';

  // Add characters one by one until it overflows
  for (let i = 0; i < fullText.length; i++) {
    clone.textContent += fullText[i];
    if (clone.scrollWidth > clone.offsetWidth) {
      // Once we've gone too far, remove the last character and break
...
mailing_dates.forEach((date_item) => {
  ...
  if (dateString) {
    ...
    if (parts.length === 3) { // Ensure it's a valid date format
      let boldPart = document.createElement('b'); // Create a new bold tag
      boldPart.textContent = `${parts[0]}.${parts[1]}`; // Set day and month
      dateTag.innerHTML = ''; // Clear the existing content
      dateTag.appendChild(boldPart); // Append the bold part
      dateTag.appendChild(document.createTextNode(`.${parts[2]}`)); // Append the year part
    }
  }
})
